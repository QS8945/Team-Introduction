;(function (root, factory) {
  ...
  baidu.object.extend(LuShu.prototype, {
    ...
    _setPosition(index) {
      this._dispatchEvent(index)
      var me = this,
        timer = 10,
        maxIndex =
          index >= this._path.length - 1 ? this._path.length - 2 : index,
        step = this._opts.speed / (1000 / timer),
        init_pos = this._projection.lngLatToPoint(me._path[maxIndex]),
        target_pos = this._projection.lngLatToPoint(me._path[maxIndex + 1]),
        count = Math.round(me._getDistance(init_pos, target_pos) / step),
        x = me._tween.linear(
          init_pos.x,
          target_pos.x,
          index >= this._path.length - 1 ? count : 0,
          count
        ),
        y = me._tween.linear(
          init_pos.y,
      ...
      if (me._opts.enableRotation == true) {
        me.setRotation(proPos, me._path[maxIndex], me._path[maxIndex + 1])
      }
      ...
      for (var i = 0, len = t.length; i < len; i++) {
        if (!t[i].bShow) {
          distance = this._map.getDistance(
            new BMap.Point(t[i].lng, t[i].lat),
            markerPoi
          )
          if (distance < 10) {
            t[i].bShow = true
